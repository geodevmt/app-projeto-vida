...
export default function LoginPage() {
  ...
  const onSubmit = async (data: LoginForm) => {
    ...
    try {
      ...
      router.push("/dashboard"); // O middleware redirecionará se for admin
    } catch (error: any) {
      ...
      <Card className="w-full max-w-md shadow-lg border-t-4 border-blue-600">
        ...
        <CardContent>
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div className="space-y-2">
              ...
              <Input id="email" type="email" placeholder="nome@escola.mt.gov.br" {...register("email")} />
              {errors.email && <p className="text-sm text-red-500">{errors.email.message}</p>}
            </div>
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <Label htmlFor="password">Senha</Label>
                {/* Link para recuperar senha (implementaremos futuramente) */}
        ...
        <CardFooter className="flex justify-center bg-gray-50 py-4 rounded-b-lg">
          <p className="text-sm text-gray-500">
            É um aluno novo? <a href="/cadastro" className="text-blue-700 font-semibold hover:underline">Criar conta de estudante</a>
          </p>
        </CardFooter>
      </Card>
    </div>
